 ...
 * @date 09/28/2022
 * 
 * @description	TMTC0006826 Document Reversal Reject - FB08 & FBRA
 */
package com.lexmark;
...
public class DocumentReversalReject extends Parent {
	...
	public void TS02CreateDocumentReversalRequest() {
		try {
			...
			writerOutput = new FileWriter("output/DocumentReversalReject.csv", false);
			...
			outputData = new String[] { "TMTC0006826 Document Reversal Reject - FB08 & FBRA", "Failed",
	...
	public void TS04RejectDocumentReversalRequest() {
		try {
			writerOutput = new FileWriter("output/DocumentReversalReject.csv", false);
			writer = new CSVWriter(writerOutput);
			Response response = RestAssured.given()
					...
					.queryParam("RejectionComment", "'Test API Reject'").queryParam("WiId", "'" + WiId + "'")
					.queryParam("ZZ_DRFB_NO", "'" + DocRevReq + "'").when().post("/RejectDRFBRequest").then()
					.statusCode(200).statusLine("HTTP/1.1 200 OK").extract().response();
			Assert.assertTrue(response.jsonPath().get("d.RejectDRFBRequest.Success"));
			outputData = new String[] { "TMTC0006826 Document Reversal Reject - FB08 & FBRA", "Passed",
					"Document Reversal Request: " + DocRevReq };
			writer.writeNext(outputData, false);
			writer.close();
		} catch (Exception e) {
